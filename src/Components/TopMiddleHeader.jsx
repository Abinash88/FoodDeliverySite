...
const TopMiddleHeader = () => {
  ...
  return (
    <div className="topMiddleHeader w-[90%] pb-5 flex justify-between items-center mx-auto mt-10">
      <h3
        className={`${
          ToggleDarkMode ? "text-gray-100" : "text-gray-900"
        } md:text-[19px] text-[16px] font-bold`}
      >
        Hello, Samanatha
      </h3>
      ...
      <div className="deliveryPickupBtn sm:mr-0 mr-7 h-[40px] bg-gray-300 cursor-pointer relative lg:w-[150px] sm:w-[140px] w-[100px] flex rounded-lg bg-white">
        <div
          ...
          className={`w-[50%]  z-10 ${
            selectPart ? "text-gray-600" : "bg-yellow-500 text-white"
          }  h-full  rounded-l-lg text-center  font-semibold text-[12px] lg:text-[14px] flex items-center  justify-center`}
        >
          <ArchiveBoxIcon
            className={`h-6 ${
              selectPart ? "text-gray-600" : " text-white"
            } sm:hidden block `}
          />
        ...
        <div
          ...
          className={`w-[50%]   ${
            selectPart ? "bg-yellow-500 text-white" : "text-gray-600"
          }   rounded-r-lg  h-full flex justify-center items-center text-center font-semibold text-[12px] lg:text-[14px]`}
        >
          <TruckIcon
            className={`h-6 ${
              selectPart ? " text-white" : "text-gray-600"
            } sm:hidden block text-`}
          />
          <span className="sm:block hidden">Pickup</span>
        </div>

        <div
          onClick={CheckRightHeader}
          className="hamburgermenu xl:hidden block fixed z-50 right-[10px] top-10 cursor-pointer hover:bg-gray-200 rounded-md p-[1px]"
        >
      ...
      <form className="lg:w-[300px] w-auto formclass lg:rounded-lg rounded-full sm:flex hidden  h-[40px] bg-white flex items-center  px-2">
        ...
        <input
          ...
          className="w-full h-full bg-transparent lg:block hidden rounded-lg"
          name=""
        />
      </form>
    </div>
